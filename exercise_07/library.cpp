...
// #include <windows.h> window에서만 가능 맥에서는 달라짐
...
void add_song(char *artist, char *title, char *path){
    ...
    // add the song to the artist pointed by artist
...
void insert_to_index_directory(Song *ptr_song){
    ...
    // insert the snode into the single linked at index_table[index]
...
void insert_node(Artist *ptr_artist, Snode *ptr_snode){
    ...
    else if(p == ptr_artist -> head){ // 2 at the front
        ptr_snode -> next = ptr_artist -> head;
        ptr_artist -> head -> prev = ptr_snode;
        ptr_artist -> head = ptr_snode;
    }
    else if(p == NULL){ // 3 at the end
...
void remove_song(int index){
    ...
    if(q == NULL){ //remove first
        ...
        // p = p->next 이건 장소와 값을 혼동하는것

    }
    else{ // remove after q
        q->next = p->next; // q->next = q->next->next 와 동일
    }
    free(p);

    Artist *ptr_artist = ptr_song->artist;
    // find the snode in the double linked list od ptr_artist
